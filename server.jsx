...
const AppContext = createContext({lang: 'de', t: {}, authenticated: false});

/*
 * Utils
 */

function getLanguage(c, i18n) {
  // 1. Check URL query parameter and set cookie if valid
  const query = c.req.query('lang');
  if (query) {
    const lang = query.toLowerCase();
  ...
  const preferredLanguage = accepts(c, {
    header: 'Accept-Language',
    supports: availableLanguages,
    default: 'en',
  });
  ...
  // 4. Return default language if it exists in i18n, otherwise first available language
  if (Object.prototype.hasOwnProperty.call(i18n, 'en')) {
    return 'en';
  }

  return Object.keys(i18n)[0];
}
